 ...
 **/
@Controller
@RequestMapping(value = "/aispeaker")
public class SnsLoginController {
     ...
     * 기본 주소.
     */
    private final String thisUrl = "aispeaker";
    ...
    // http://localhost:8889/aispeaker/snscallback,    encodeURIComponent('http://localhost:8889/aispeaker/snscallback')
    //개발
//    private static final String callBackURL = "http%3A%2F%2Fpips.daewooenc.com%3A8889%2Faispeaker%2Fsnscallback";
    // 상용
//    private String callBackURL = "http%3A%2F%2F"+prugioDomain+"%3A8889%2Faispeaker%2Fsnscallback";
//    private static final String naverCallBackURL = "https%3A%2F%2Fauth.expo.io%2Faispeaker%2Fsnscallback";
//    private static final String kakaoCallBackURL = "http://localhost/8889/aispeaker/snscallback";
    // 개인
    /*
...
//    private static final String naverRequestUrl = "https://nid.naver.com/oauth2.0/authorize?client_id=" + naverClientId + "&response_type=code&redirect_uri="+ naverCallBackURL + "&state=";
    ...
    public String insert(HttpServletRequest request, CommunicationAuthCode communicationAuthCode) {
    ...
    public String oauth(Model model, HttpServletRequest request, CommunicationAuthCode communicationAuthCodeDB) {
        ...
        String callBackURL = "http%3A%2F%2F"+prugioDomain+"%3A8889%2Faispeaker%2Fsnscallback";

        if(request.isSecure()){
            callBackURL = "https%3A%2F%2F"+prugioDomain+"%3A18889%2Faispeaker%2Fsnscallback";
        } else {
            callBackURL = "http%3A%2F%2F"+prugioDomain+"%3A8889%2Faispeaker%2Fsnscallback";
        }
        ...
        if ("NORMAL".equals(snsType)) {
            ...
            // id 조회
            existLoginID = snsService.existNormalID(userId, new Sha256Cipher(password).encrypt());
        ...
        if(commnunityState != null && !"".equals(commnunityState)) {
            if ("naver".equals(snsType)) {
                ...
                logger.debug("snsAuthCode================" + snsAuthCode);
                    ...
                    String redirect_uri = "http://smarthome.prugio.com:8889/aispeaker/snscallback";
                    if(request.isSecure()){
                        redirect_uri = "https://smarthome.prugio.com:18889/aispeaker/snscallback";
                    } else {
                        redirect_uri = "http://smarthome.prugio.com:8889/aispeaker/snscallback";
                    }
                ...
                logger.debug("callBackErrorCode================[" + callBackErrorCode + "]");
                    ...
                    if (!"error".equals(callBackErrorCode) && callBackErrorCode == null && !"".equals(callBackErrorCode)) {
                        String accessTokenReqUrl = getAccessTokenUrl(commnunityState, snsAuthCode, snsType, "");
                        logger.debug("accessTokenReqUrl================" + accessTokenReqUrl);
                        //String redirect_uri = "http://localhost:8080/aispeaker/snscallback";
                        ...
                        redirect_uri = "http://pips.daewooenc.com:8889/aispeaker/snscallback";
                        ...
                        try {
                            ...
                            if (!jsonObject.has("error")) {
                                ...
                                if ("200".equals(httpResult.getStatus())) {
                                    ...
                                    logger.error("SNS Profile Search Fail. error code : " + httpResult.getStatus() + " description : " + jsonObject.get("error_description"));
                                    logger.debug("SNS Profile Search Fail. error code : " + httpResult.getStatus() + " description : " + jsonObject.get("error_description"));
                        ...
                        logger.debug("SNS 연동 실패 Error Code : " + callBackErrorCode);
                        model.addAttribute("resultMsg", resultMsg);
                        return "snsexception";
                    }
            } else if ("sk".equals(snsType)) {
            ...
            if (snsID != null && !"".equals(snsID)) {
                // id 조회
                existSnsID = snsService.existSnsID(snsID);
